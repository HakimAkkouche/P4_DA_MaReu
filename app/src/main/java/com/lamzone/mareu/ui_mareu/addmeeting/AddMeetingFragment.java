...
import com.lamzone.mareu.ui_mareu.addmeeting.viewmodel.AddMeetingViewModel.AddMeetingViewAction;
...
public class AddMeetingFragment extends Fragment{
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        mViewModel.getViewStateLiveData().observe(getViewLifecycleOwner(), viewState -> {
            mBinding.addMeetingName.setError(viewState.getMeetingNameError());
        ...
        mViewModel.getViewActionLiveData().observe(getViewLifecycleOwner(), addMeetingViewAction -> {
            if (addMeetingViewAction instanceof AddMeetingViewAction.DisplayDateTimePicker) {
                initDatePicker();
            }
        });
        mBinding.addSaveButton.setOnClickListener(v -> {
            if(mParticipants.size() == 1){
                ...
                if(mViewModel.createMeeting()){
                    Navigation.findNavController(v).navigate(R.id.action_AddFragment_to_MeetingsFragment);
                }
            }
        });
